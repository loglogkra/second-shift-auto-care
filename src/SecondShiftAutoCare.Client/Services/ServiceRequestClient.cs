...
public sealed class ServiceRequestClient(HttpClient http)
{
    ...
    {
        var response = await http.GetAsync("api/manage/service-requests");
    ...
    {
        var response = await http.GetAsync($"api/manage/service-requests/{id}");
    ...
    public Task<ServiceRequestDto> UpdateStatusAsync(Guid id, ServiceRequestStatusUpdateModel model) =>
        PatchAsync($"api/manage/service-requests/{id}/status", model, "save status");

    public Task<ServiceRequestDto> UpdateQuoteAsync(Guid id, ServiceRequestQuoteUpdateModel model) =>
        PutAsync($"api/manage/service-requests/{id}/quote", model, "save quote");

    public Task<ServiceRequestDto> UpdateNotesAsync(Guid id, ServiceRequestNotesUpdateModel model) =>
        PutAsync($"api/manage/service-requests/{id}/notes", model, "save notes");
    ...
    public async Task<List<JobChecklistItemDto>> GetChecklistAsync(Guid id) => await http.GetFromJsonAsync<List<JobChecklistItemDto>>($"api/manage/service-requests/{id}/checklist") ?? [];
    public async Task<JobChecklistItemDto> AddChecklistItemAsync(Guid id, ChecklistItemCreateModel model) => await ReadAsync<JobChecklistItemDto>(await http.PostAsJsonAsync($"api/manage/service-requests/{id}/checklist", model), "add checklist item");
    public async Task<JobChecklistItemDto> UpdateChecklistItemAsync(Guid id, Guid itemId, ChecklistItemUpdateModel model) => await ReadAsync<JobChecklistItemDto>(await http.PutAsJsonAsync($"api/manage/service-requests/{id}/checklist/{itemId}", model), "update checklist item");
    public async Task DeleteChecklistItemAsync(Guid id, Guid itemId) => await EnsureSuccessAsync(await http.DeleteAsync($"api/manage/service-requests/{id}/checklist/{itemId}"), "delete checklist item");
    public Task<ServiceRequestDto> UpdateScheduleAsync(Guid id, ServiceRequestScheduleUpdateModel model) => PutAsync($"api/manage/service-requests/{id}/schedule", model, "save schedule");
    public async Task<PaymentDto> GetPaymentAsync(Guid id) => await ReadAsync<PaymentDto>(await http.GetAsync($"api/manage/service-requests/{id}/payment"), "load payment");
    public async Task<PaymentDto> UpdatePaymentAsync(Guid id, PaymentDto model) => await ReadAsync<PaymentDto>(await http.PutAsJsonAsync($"api/manage/service-requests/{id}/payment", model), "save payment");
    public async Task<JobRiskAssessmentDto> GetRiskAsync(Guid id) => await ReadAsync<JobRiskAssessmentDto>(await http.GetAsync($"api/manage/service-requests/{id}/risk-assessment"), "load risk assessment");
    public async Task<JobRiskAssessmentDto> UpdateRiskAsync(Guid id, JobRiskAssessmentDto model) => await ReadAsync<JobRiskAssessmentDto>(await http.PutAsJsonAsync($"api/manage/service-requests/{id}/risk-assessment", model), "save risk assessment");
    ...
    public async Task<CustomerHistoryDto?> GetCustomerAsync(Guid id) => await http.GetFromJsonAsync<CustomerHistoryDto>($"api/manage/customers/{id}");
    public async Task<VehicleHistoryDto?> GetVehicleAsync(Guid id) => await http.GetFromJsonAsync<VehicleHistoryDto>($"api/manage/vehicles/{id}");
